...
(function() {
	...
	ixmaps.legend.makeColorLegendHTML = function(szId,szLegendId,szMode){
		...
		// color legend = table
		// ---------------------
		szHtml += "<div id='legend-classes"+szLegendId+"' style='border-spacing:2px;line-height:1.25em;margin-bottom:1.5em;max-height:300px;padding-right:0.5em;overflow:auto' >";
		...
		if ( fColorScheme &&
			...
			for ( var i=0; i<nRows; i++){
				...
				if ( fSelected ){
					szHtml += "<div valign='center' class='theme-legend-item-selected'>";
				}else{
					szHtml += "<div valign='center' class='theme-legend-item' >";
				}

				var szLabel = labelA[sortA[i].index];
				szHtml += "<span class='theme-legend' >";
				szHtml += "<a class='theme-button' href='#' title='click to see'>";
				szHtml += "<span title='"+szLabel+"' style='color:black'>";
				szHtml += "<span onclick='javascript:ixmaps.markThemeClass(\""+szId+"\","+sortA[i].index+");event.stopPropagation();return false;' >";
				szHtml += szLabel;
				szHtml += "</span>";
				szHtml += "</span>";
				szHtml += "</a>";

				szHtml += sortA[i].count?("<span class='theme-legend-count' style='color:#888888;float:right;padding-top:0.5em'> "+count+" "+szUnit.replace(/ /g,'&nbsp;')+"</span>"):"";;

				szHtml += "</span>";

				szHtml += "</div><div style='margin-left:1px;margin-top:-5px;margin-bottom:0px;padding-bottom:0px;border-bottom:white solid 0.5px;'>";				
				
				var barsize = 0.5;
				szHtml += "<span style='font-size:"+barsize+"em'>";

				szHtml += "<a class='legend-color-button' href='#' title='click to see'>";
				szHtml += "<span onclick='javascript:ixmaps.markThemeClass(\""+szId+"\","+sortA[i].color+");event.stopPropagation();' style='background:"+colorA[sortA[i].color]+";opacity:0.7'>";
	...
	ixmaps.legend.makeColorLegendHTMLCompact = function(szId,szLegendId){
		...
		// show theme filter, if defined
		if (themeObj.szFilter){
			szHtml += "<p class='legend-filter' style='margin-top:0em;color:#fff;background:#ddd'><span class='icon icon-filter' style='float:left;padding:0.2em 0.5em;'></span><span class='legend-filter-text'>"+themeObj.szFilter+"</span><a href='javascript:ixmaps.changeThemeStyle(null,\"filter\",\"remove\");' title=\"remove\" ><span class='icon icon-cancel-circle' style='float:right;padding:0.2em 0.5em;'></span></a></p>";
		}
