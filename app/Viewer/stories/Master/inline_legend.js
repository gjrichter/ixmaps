...
(function() {
	...
	ixmaps.legend.makeColorLegendHTML = function(szId,szLegendId,szMode){
		...
		// color legend = table
		// ---------------------
		szHtml += "<div id='legend-classes"+szLegendId+"' style='border-spacing:2px;line-height:1.25em;margin-top:0.2em;margin-bottom:1.5em' >";
		...
		if ( fColorScheme &&
			...
			for ( var i=0; i<nRows; i++){
				...
				// show class count
				var szCount = ixmaps.__formatValue(sortA[i].count,2,"BLANK");
				...
				if ( colorA[sortA[i].color] == "none" )	{
					...
					szHtml += "<span style='background:"+colorA[sortA[i].color]+";opacity:0.7;font-size:0.5em'>";
				}
	...
	ixmaps.legend.makeColorLegendHTMLCompact = function(szId,szLegendId){
		...
		// show theme filter, if defined
		if (themeObj.szFilter){
			szHtml += "<p class='legend-filter' ><span class='icon icon-filter' style='float:left;padding:0.2em 0.5em;'></span><span class='legend-filter-text'>"+themeObj.szFilter+"</span><a href='javascript:ixmaps.changeThemeStyle(null,\"filter\",\"remove\");' title=\"remove\" ><span class='icon icon-cancel-circle' style='float:right;padding:0.2em 0.5em;'></span></a></p>";
		}
