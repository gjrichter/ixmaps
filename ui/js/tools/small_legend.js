...
(function() {
	...
	ixmaps.legend.makeColorLegendHTMLCompact = function(szId,szLegendId){
		...
		for ( var row=0; row<nRows; row++){
			...
			for ( var i=0; i<colorA.length; i++){
				...
				szHtml += "<td><a class='theme-button' href='#' title='"+szMinMax+" click to see'><span onclick='javascript:ixmaps.markThemeClass(\""+szId+"\","+ix+");event.stopPropagation();return false;' style='background:"+colorA[ix]+";font-size:0.7em;opacity:"+nOpacity+"'>";
	...
	__sizeLegendDialog = function(szId){
		...
		var newWidth = Math.max($("#legend-classes"+szId).width()+60,$("#legend-buttons"+szId).width()+50);
