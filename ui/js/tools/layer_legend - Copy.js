...
(function () {
				...
				switch( layer.szType ){
					...
					case "polygon":
						for ( c in layer.categoryA ){
							...
							szLegend += "<div class='list-group-item-right' style='width:95%;'>";
							szLegend += "<span style='reset:both;color:"+(szChecked.length?"inherit":"#bbb")+"'>"+szCatogoryName+"</span>";
							szLegend += "</div>";
							szLegend += '<div class="checkbox checkbox-right">';
							szLegend += '<input type="checkbox" class="check sub_check" '+szChecked+' style="margin:0.4em 0em 0 0.2em;float:left" onchange="javascript:ixmaps.__switchLayer($(this),\''+name+((c&&(layer.categoryA[c].legendname))?('::'+String(c).replace(/\'/g,"\\\'")):'')+'\');">&nbsp;';
							szLegend += '<span class="cr"><i class="cr-icon fa fa-check fa-fw"></i></span>';
							szLegend += "</div>";
		...
		ixmaps.makeLayerLegend = function(nMaxHeight){
			...
			$("div.checkbox-right").css(checkbox_right);
			$("ul").css(ul_);
