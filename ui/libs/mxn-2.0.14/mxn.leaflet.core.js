...
Mapstraction: {
    ...
    addControls: function(args) {
        var map = this.maps[this.api];
        if (args.zoom) {
			if ( mxn.lastZoomControl ){
				map.removeControl(mxn.lastZoomControl);
			}
            ...
            map.addControl(zoom);
			mxn.lastZoomControl = zoom;
        }else{
			if ( mxn.lastZoomControl ){
				map.removeControl(mxn.lastZoomControl);
				mxn.lastZoomControl = null;
			}
		}
        if (args.map_type) {
            var layersControl = new L.Control.Layers(this.layers, this.features);
            map.addControl(layersControl);
			mxn.lastLayerControl = layersControl;
        }else{
			if ( mxn.lastLayerControl ){
	            map.removeControl(mxn.lastLayerControl);
			}
		}
    },
